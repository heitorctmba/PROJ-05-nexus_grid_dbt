...
/*
    ============================================================================
    STAGING MODEL: stg_logger_analogic_data
    ============================================================================

    Descrição:
        Extrai e normaliza dados analógicos dos smart loggers a partir da
        tabela raw_logger. Converte valores do formato Grid Co (valor@timestamp)
        para valores numéricos puros

    Arquitetura:
        Camada: STAGING (Silver Layer)
        Fonte: raw_logger
        Destino: stg_logger_analogic_data (Hypertable TimescaleDB)
...
extracted_analogic AS (
    SELECT DISTINCT ON (device_id, ((json_data->>'timestamp')::TIMESTAMPTZ - INTERVAL '3 hours')::TIMESTAMP)
        ...
        {{ extract_grid_value('json_data', 'daily_active_energy') }} AS daily_active_energy,
        {{ extract_grid_value('json_data', 'cumulative_active_energy') }} AS cumulative_active_energy,
        {{ extract_grid_value('json_data', 'power_reactive') }} AS power_reactive,
        {{ extract_grid_value('json_data', 'dc_input_power') }} AS dc_input_power,
        {{ extract_grid_value('json_data', 'active_power_limit') }} AS active_power_limit,
        {{ extract_grid_value('json_data', 'power_factor') }} AS power_factor,

        -- ====================================================================
        -- CAMPOS ANALÓGICOS - EFICIÊNCIA E CORRENTE DC
        -- ====================================================================
        {{ extract_grid_value('json_data', 'efficiency') }} AS efficiency,
        {{ extract_grid_value('json_data', 'dc_current') }} AS dc_current,

        -- ====================================================================
        -- CAMPOS ANALÓGICOS - CORRENTES DE FASE
        -- ====================================================================
        ...
        -- ====================================================================
        -- CAMPOS ANALÓGICOS - TENSÕES DE LINHA
        -- ====================================================================
        ...
        -- ====================================================================
        -- CAMPOS ANALÓGICOS - CONTADORES
        -- ====================================================================
        {{ extract_grid_value('json_data', 'inverters_producing_count', 'INTEGER') }} AS inverters_producing_count,

        -- ====================================================================
        -- CAMPOS DISCRETOS - ESTADOS E ALARMES
        -- ====================================================================
        {{ extract_grid_value('json_data', 'logger_state', 'INTEGER') }} AS logger_state,
        {{ extract_grid_value('json_data', 'operation_state', 'INTEGER') }} AS operation_state,
        {{ extract_grid_value('json_data', 'breaker_position', 'INTEGER') }} AS breaker_position
